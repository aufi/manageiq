...
describe RequestRefererService do
  ...
  describe "#referer_valid?" do
    ...
    describe "when the referer starts with the given string" do
      ...
      it "returns true" do
        expect(request_referer_service.referer_valid?(referer, string_to_test, useragent, controller, action)).to be_true
      end
    end
    ...
    describe "when the referer does not start with the given string" do
      ...
      it "returns false" do
        expect(request_referer_service.referer_valid?(referer, string_to_test, useragent, controller, action)).to be_false
      end
  ...
  describe '#access_whitelisted?' do
    it "allows only GET" do
      expect(request_referer_service.access_whitelisted?(get_request,   controller_name, action_name)).to be_true
      expect(request_referer_service.access_whitelisted?(post_request,  controller_name, action_name)).to be_false
    end

    it "allows only whitelistet entry points" do
      expect(request_referer_service.access_whitelisted?(get_request, controller_name, dissallowed_action_name)).to be_false
      expect(request_referer_service.access_whitelisted?(get_request, dissallowed_controller_name, action_name)).to be_false
    end

    it "requires an 'id' when specified" do
      expect(request_referer_service.access_whitelisted?(get_request_no_id, controller_name, action_name)).to be_false
    end

    it "requires no params, where none specified" do
      expect(request_referer_service.access_whitelisted?(get_request_no_id, controller_name_no_params, action_name_no_params)).to be_true
    end

    it "disallows xml_http requests" do
      expect(request_referer_service.access_whitelisted?(get_request_xml_http, controller_name, action_name)).to be_false
    end
  end
end

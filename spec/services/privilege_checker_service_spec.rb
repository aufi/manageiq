...
describe PrivilegeCheckerService do
  ...
  describe "#valid_session?" do
    shared_examples_for "PrivilegeCheckerService#valid_session? that returns false" do
      it "returns false" do
        expect(privilege_checker.valid_session?(session, user)).to be_false
      end
    end
    ...
    context "when the user is signed in" do
      ...
      context "when the session has not timed out" do
        ...
        context "when the server is ready" do
          ...
          it "returns true" do
            expect(privilege_checker.valid_session?(session, user)).to be_true
          end
        end
      end
    end
  ...
  describe "#user_session_timed_out?" do
    ...
    context "when a user exists" do
      ...
      context "when the session is timed out" do
        ...
        it "returns true" do
          expect(privilege_checker.user_session_timed_out?(session, user)).to be_true
        end
      end
      ...
      context "when the session has not timed out" do
        ...
        it "returns false" do
          expect(privilege_checker.user_session_timed_out?(session, user)).to be_false
        end
      end
    end

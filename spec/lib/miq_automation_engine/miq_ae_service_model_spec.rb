  ...
  describe MiqAeMethodService::MiqAeServiceVm do
    ...
    describe "#tag_assign" do
      ...
      it "can assign an exiting tag to ae_vm" do
        expect(@ae_vm.tag_assign("#{category.name}/#{tag.name}")).to be_true
        expect(@ae_vm.tagged_with?(category.name, tag.name)).to be_true
      end

      it "cannot assign a non-existing tag to ae_vm, but no error is raised" do
        expect(@ae_vm.tag_assign("#{category.name}/non_exisiting_tag")).to be_true
        expect(@ae_vm.tagged_with?(category.name, 'non_exisiting_tag')).to be_false
      end
    end
    ...
    describe "#tag_unassign" do
      ...
      context "with assigned tags" do
        ...
        it "can unassign a tag from ae_vm" do
          expect(@ae_vm.tag_unassign("#{category.name}/#{tag.name}")).to be_true
          expect(@ae_vm.tagged_with?(category.name, tag.name)).to be_false
        end

        it "unassigns only specified tag from ae_vm but not other tags from the same category" do
          expect(@ae_vm.tag_assign("#{category.name}/#{another_tag.name}")).to be_true

          expect(@ae_vm.tag_unassign("#{category.name}/#{tag.name}")).to be_true
          expect(@ae_vm.tagged_with?(category.name, another_tag.name)).to be_true
        end
      end

      it "does not raise an error when attempts to unassign a non-existing tag" do
        expect(@ae_vm.tag_unassign("#{category.name}/non_exisiting_tag")).to be_true
      end
    end
  end

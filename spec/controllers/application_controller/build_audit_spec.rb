...
describe ApplicationController do
  context "audit events" do
    it "tests build_config_audit" do
      ...
      expect(controller.send(:build_config_audit, edit[:new], edit[:current])).to eq {:event   => "vmdb_config_update",
                          :userid  => nil,
    ...
    it "tests build_created_audit" do
      ...
      edit = {:new     => {:name => "the-name", :changing_value => "test2", :static_value => "same",
                           ...
                           :password => "pw1"}}
      expect(controller.send(:build_created_audit, category, edit)).to eq {:event        => "classification_record_add",
    ...
    it "tests build_saved_audit" do
      ...
      expect(controller.send(:build_saved_audit, category, edit)).to eq {:event        => "classification_record_update",
                          :target_id    => category.id,
                          :target_class => category.class.name,
                          :userid       => nil,

...
describe Metric do
        ...
        context "collecting vm realtime data" do
          ...
          it "should have collected counters and values" do
            ...
            until ts > timestamps.last
              expect(counter_values.key?(ts)).to be_true
  ...
  def assert_infra_targets_enabled(targets, expected_types)
    ...
    targets.each do |t|
      ...
      expect(expected_enabled).to be_true
    end
  ...
  def assert_perf_capture_now(target, mode)
    Timecop.freeze(Time.now) do
      ...
      expect(target.perf_capture_now?).to be_true
      ...
      expect((mode == :with_alerts ? target.perf_capture_now? : !target.perf_capture_now?)).to be_true

      target.update_attribute(:last_perf_capture_on, Time.now.utc - 1.minutes)
      expect(target.perf_capture_now?).not_to be_true
    end
  end
end

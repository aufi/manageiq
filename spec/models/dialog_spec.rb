...
describe Dialog do
  ...
  context "#destroy" do
    ...
    it "destroy without resource_action association" do
      expect(@dialog.destroy).to be_true
  ...
  context "#add_resource" do
    ...
    it "dialog contain tabs" do
      ...
      @dialog.save
      expect(@dialog.dialog_tabs).to have(1).thing
    end

    it "tabs contain groups" do
      @dialog_tab.add_resource(@dialog_group)
      @dialog_tab.save
      expect(@dialog_tab.dialog_groups).to have(1).thing
    end

    it "groups contain fields" do
      @dialog_group.add_resource(@dialog_field)
      @dialog_group.save
      expect(@dialog_group.dialog_fields).to have(1).thing
    end
  end
  ...
  context "#add_resource!" do
    ...
    it "add controls" do
      ...
      @dialog_group.reload
      expect(@dialog_group.dialog_fields).to have(5).things
    end
  end
  ...
  context "#remove_all_resources" do
    ...
    it "dialogs contain tabs" do
      @dialog.add_resource(@dialog_tab)
      expect(@dialog.dialog_resources).to have(1).thing
      @dialog.remove_all_resources
      expect(@dialog.dialog_resources).to have(0).things
    end

    it "tabs contain groups" do
      @dialog_tab.add_resource(@dialog_group)
      expect(@dialog_tab.dialog_resources).to have(1).thing
      @dialog_tab.remove_all_resources
      expect(@dialog_tab.dialog_resources).to have(0).things
    end

    it "groups contain fields" do
      @dialog_group.add_resource(@dialog_field)
      expect(@dialog_group.dialog_resources).to have(1).thing
      @dialog_group.remove_all_resources
      expect(@dialog_group.dialog_resources).to have(0).things
    end
  end

...
describe Service do
  ...
  context "with a small env" do
    ...
    it "should create a valid service" do
      ...
      expect(@service.service_resources).to have(0).things
    end
    ...
    it "should allow a resource to be connected only once" do
      ...
      expect(@service.service_resources).to have(2).things
    end
    ...
    it "should set the group index when adding a resource" do
      ...
      expect(@service.group_has_resources?(1)).to be_true
      expect(@service.group_has_resources?(0)).to be_false
    end
    ...
    context "with VM resources" do
      ...
      it "should remove all connected resources" do
        expect(@service.service_resources).not_to have(0).things
        @service.remove_all_resources
        expect(@service.service_resources).to have(0).things
      end

      it "should check if a group index has resources" do
        expect(@service.group_has_resources?(0)).to be_true
        expect(@service.group_has_resources?(1)).to be_false
        @service.remove_all_resources
        expect(@service.group_has_resources?(0)).to be_false
        expect(@service.group_has_resources?(1)).to be_false
      end

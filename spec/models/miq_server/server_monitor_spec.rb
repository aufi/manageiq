  ...
  context "After Setup," do
    ...
    it "should respond properly to UI helper methods" do
      @server_roles.each do |server_role|
        expect(server_role.unlimited?).to be_true        if server_role.max_concurrent == 0
        expect(server_role.master_supported?).to be_true if server_role.max_concurrent == 1
    ...
    context "with 1 Server" do
      ...
      it "should activate unlimited zone role via activate_in_zone method" do
        ...
        expect(@miq_server.active_role_names.include?(rolename)).to be_true
        expect(@miq_server.inactive_role_names.include?(rolename)).not_to be_true
      end
      ...
      context "after initial monitor_servers" do
        ...
        it "should have all roles active after monitor_servers" do
          ...
          expect(@miq_server.active_role_names.include?("ems_operations")).to be_true
          expect(@miq_server.active_role_names.include?("event")).to be_true
          expect(@miq_server.active_role_names.include?("scheduler")).to be_true
          expect(@miq_server.active_role_names.include?("reporting")).to be_true
        end
        ...
        it "should deactivate unlimited role via deactivate_in_zone method" do
          ...
          expect(@miq_server.active_role_names.include?(rolename)).not_to be_true
          expect(@miq_server.inactive_role_names.include?(rolename)).to be_true
        end
        ...
        it "should activate newly assigned unlimited zone role" do
          ...
          expect(@miq_server.active_role_names.include?("scheduler")).to be_true
          expect(@miq_server.active_role_names.include?("reporting")).to be_true
          expect(@miq_server.active_role_names.include?("smartstate")).to be_true
        end
        ...
        it "should activate newly assigned limited zone role" do
          ...
          expect(@miq_server.active_role_names.include?("scheduler")).to be_true
          expect(@miq_server.active_role_names.include?("reporting")).to be_true
          expect(@miq_server.active_role_names.include?("ems_inventory")).to be_true
        end
        ...
        it "should activate newly assigned unlimited region role" do
          ...
          expect(@miq_server.active_role_names.include?("scheduler")).to be_true
          expect(@miq_server.active_role_names.include?("reporting")).to be_true
          expect(@miq_server.active_role_names.include?("database_operations")).to be_true
        end
        ...
        it "should activate newly assigned limited region role" do
          ...
          expect(@miq_server.active_role_names.include?("scheduler")).to be_true
          expect(@miq_server.active_role_names.include?("reporting")).to be_true
          expect(@miq_server.active_role_names.include?("database_synchronization")).to be_true
        end
    ...
    context "with 2 Servers in 2 Zones where I am the Master" do
      ...
      it "should activate unlimited role via activate_in_zone method" do
        ...
        expect(@miq_server1.active_role_names.include?(rolename)).to be_true

        expect(@miq_server2.inactive_roles.length).to eq 3
        expect(@miq_server2.active_role_names.length).to eq 1
        expect(@miq_server2.active_role_names.include?(rolename)).to be_true
      end
      ...
      context "after monitor_servers" do
        ...
        it "should have all roles active after sync between them" do
          expect((@miq_server1.active_role_names.include?("ems_operations") && @miq_server2.active_role_names.include?("ems_operations"))).to be_true
          expect((@miq_server1.active_role_names.include?("event") ^ @miq_server2.active_role_names.include?("event"))).to be_true
          expect((@miq_server1.active_role_names.include?("reporting") && @miq_server2.active_role_names.include?("reporting"))).to be_true
          expect((@miq_server1.active_role_names.include?("scheduler") ^ @miq_server2.active_role_names.include?("scheduler"))).to be_true
        end
      end
      ...
      context "with Non-Master having the active roles" do
        ...
        it "should have all roles on the desired servers" do
          expect((@miq_server1.active_role_names.include?("ems_operations") && @miq_server2.active_role_names.include?("ems_operations"))).to be_true
          expect((@miq_server1.inactive_role_names.include?("event") && @miq_server2.active_role_names.include?("event"))).to be_true
        end
        ...
        context "where Non-Master shuts down cleanly" do
          ...
          it "should migrate roles to Master" do
            expect(@miq_server1.active_role_names.include?("ems_operations")).to be_true
            expect(@miq_server1.active_role_names.include?("event")).to be_true
        ...
        context "where Non-Master is not responding" do
          ...
          it "should migrate roles to Master" do
            ...
            expect(@miq_server1.active_role_names.include?("ems_operations")).to be_true
            expect(@miq_server1.active_role_names.include?("event")).to be_true
            expect(@miq_server1.active_role_names.include?("reporting")).to be_true
            expect(@miq_server1.active_role_names.include?("scheduler")).to be_true
          end
        end
      end
    end
    ...
    context "with 2 Servers where I am the non-Master" do
      ...
      context "where Master shuts down cleanly" do
        ...
        it "should takeover as Master" do
          expect(@miq_server1.is_master?).to be_true
          @miq_server2.reload
          expect(@miq_server2.is_master?).not_to be_true
        end
      ...
      context "where Master is not responding" do
        ...
        it "should migrate roles to Master" do
          ...
          expect(@miq_server1.inactive_roles.length).to eq 0
          expect(@miq_server1.active_roles.length).to eq 4
          expect(@miq_server1.active_role_names.include?("database_owner")).to be_false
    ...
    context "with 3 Servers where I am the Master" do
      ...
      it "should have all roles active after sync between them" do
        expect(@miq_server1.active_role_names.include?("ems_operations")).to be_true
        expect(@miq_server2.active_role_names.include?("ems_operations")).to be_true
        expect(@miq_server3.active_role_names.include?("ems_operations")).to be_true

        expect(@miq_server1.active_role_names.include?("event")).not_to be_true
        expect(@miq_server2.active_role_names.include?("event")).to be_true
        expect(@miq_server3.active_role_names.include?("event")).not_to be_true

        expect(@miq_server1.active_role_names.include?("ems_metrics_coordinator")).not_to be_true
        expect(@miq_server2.active_role_names.include?("ems_metrics_coordinator")).not_to be_true
        expect(@miq_server3.active_role_names.include?("ems_metrics_coordinator")).to be_true

        expect(@miq_server1.active_role_names.include?("ems_inventory")).not_to be_true
        expect(@miq_server2.active_role_names.include?("ems_inventory")).not_to be_true
        expect(@miq_server3.active_role_names.include?("ems_inventory")).to be_true
      end

      it "should respond to helper methods for UI" do
        expect(@miq_server1.is_master_for_role?("ems_operations")).not_to be_true
        expect(@miq_server2.is_master_for_role?("ems_operations")).to be_true
        expect(@miq_server3.is_master_for_role?("ems_operations")).not_to be_true

        expect(@miq_server1.is_master_for_role?("event")).not_to be_true
        expect(@miq_server2.is_master_for_role?("event")).to be_true
        expect(@miq_server3.is_master_for_role?("event")).not_to be_true

        expect(@miq_server1.is_master_for_role?("ems_inventory")).not_to be_true
        expect(@miq_server2.is_master_for_role?("ems_inventory")).not_to be_true
        expect(@miq_server3.is_master_for_role?("ems_inventory")).to be_true

        expect(@miq_server1.is_master_for_role?("ems_metrics_coordinator")).not_to be_true
        expect(@miq_server2.is_master_for_role?("ems_metrics_coordinator")).not_to be_true
        expect(@miq_server3.is_master_for_role?("ems_metrics_coordinator")).to be_true
      end
      ...
      context "when Server3 is stopped" do
        ...
        it "should migrate all roles properly" do
          ...
          expect(@miq_server1.active_role_names.include?("ems_metrics_coordinator")).to be_true
          expect(@miq_server2.active_role_names.include?("ems_metrics_coordinator")).not_to be_true

          expect(@miq_server1.active_role_names.include?("ems_inventory")).not_to be_true
          expect(@miq_server2.active_role_names.include?("ems_inventory")).to be_true
        end
    ...
    context "with 3 Servers where I am the non-Master" do
      ...
      it "should have the master on Server 2" do
        expect(@miq_server1.is_master?).not_to be_true
        expect(@miq_server2.is_master?).to be_true
        expect(@miq_server3.is_master?).not_to be_true
      end
      ...
      it "should have all roles active after sync between them" do
        ...
        expect((@miq_server1.active_role_names.include?("event") ^ @miq_server2.active_role_names.include?("event"))).to be_true
        expect((@miq_server2.active_role_names.include?("ems_metrics_coordinator") ^ @miq_server3.active_role_names.include?("ems_metrics_coordinator"))).to be_true
        expect((@miq_server1.active_role_names.include?("ems_inventory") ^ @miq_server3.active_role_names.include?("ems_inventory"))).to be_true
      end
      ...
      context "where Master shuts down cleanly" do
        ...
        it "should migrate roles to Master" do
          ...
          expect(@miq_server1.active_role_names.include?("ems_operations")).to be_true
          expect(@miq_server1.active_role_names.include?("event")).to be_true
          expect(@miq_server1.active_role_names.include?("ems_inventory")).to be_true
          expect(@miq_server3.active_role_names.include?("ems_metrics_coordinator")).to be_true
        end
      end
    ...
    context "In 2 Zones," do
      ...
      context "with 2 Servers across Zones where there is no master" do
        ...
        it "should allow only 1 Master in the Region" do
          ...
          @miq_server2.reload
          expect(@miq_server1.is_master).to be_true
          expect(@miq_server2.is_master).to be_false
        ...
        it "should allow only 1 Limited Regional Role in the Region" do
          ...
          expect((@miq_server1.active_role_names.include?("scheduler") && @miq_server2.active_role_names.include?("scheduler"))).to be_false
          expect((@miq_server1.active_role_names.include?("scheduler") ^ @miq_server2.active_role_names.include?("scheduler"))).to be_true
        end
      end
      ...
      context "with 2 Servers across Zones where I am the Master" do
        ...
        it "should have proper roles active after start" do
          ...
          expect((@miq_server1.active_role_names.include?("ems_operations") && @miq_server2.active_role_names.include?("ems_operations"))).to be_true
          expect((@miq_server1.active_role_names.include?("event") && @miq_server2.active_role_names.include?("event"))).to be_true
          expect((@miq_server1.active_role_names.include?("ems_metrics_coordinator") && @miq_server2.active_role_names.include?("ems_metrics_coordinator"))).to be_true
          expect((@miq_server1.active_role_names.include?("reporting") && @miq_server2.active_role_names.include?("reporting"))).to be_true
          expect((@miq_server1.active_role_names.include?("scheduler") && !@miq_server2.active_role_names.include?("scheduler"))).to be_true
        end

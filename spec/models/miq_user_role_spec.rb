...
describe MiqUserRole do
  ...
  context ".seed" do
    ...
    it "with existing records" do
      ...
      expect(changed.reload.read_only).to be_true
  ...
  context "testing allows methods" do
    ...
    it "should return the correct answer calling allows_*? with scope => :base)" do
      ...
      expect(MiqUserRole.allows_all?(@role1.name, :scope => :base, :identifiers => ["dashboard_admin", "dashboard_add", "dashboard_view", "policy"])).to eq false
      ...
      expect(MiqUserRole.allows_all?(@role2.name, :scope => :base, :identifiers => ["dashboard_admin", "dashboard_add", "dashboard_view", "policy"])).to eq true

      expect(MiqUserRole.allows_all?(@role3.name, :scope => :base, :identifiers => ["host_show_list", "host_scan", "host_edit"])).to eq true
      expect(MiqUserRole.allows_all?(@role3.name, :scope => :base, :identifiers => ["host_view"])).to eq false
      ...
      expect(MiqUserRole.allows_all?(@role3.name, :scope => :base, :identifiers => ["vm"])).to eq false
    ...
    it "should return the correct answer calling allows_*? with scope => :one)" do
      ...
      expect(MiqUserRole.allows_all?(@role1.name, :scope => :one, :identifiers => ["dashboard_admin", "dashboard_add", "dashboard_view", "policy"])).to eq false
      ...
      expect(MiqUserRole.allows_all?(@role2.name, :scope => :one, :identifiers => ["dashboard_admin", "dashboard_add", "dashboard_view", "policy"])).to eq false

      expect(MiqUserRole.allows_all?(@role3.name, :scope => :one, :identifiers => ["host_show_list", "host_scan", "host_edit"])).to eq false
      expect(MiqUserRole.allows_all?(@role3.name, :scope => :one, :identifiers => ["host_view"])).to eq false
      ...
      expect(MiqUserRole.allows_all?(@role3.name, :scope => :one, :identifiers => ["vm"])).to eq false
    ...
    it "should return the correct answer calling allows_*? with default scope => :sub" do
      ...
      expect(MiqUserRole.allows_all?(@role1.name, :identifiers => ["dashboard_admin", "dashboard_add", "dashboard_view", "policy"])).to eq false

      expect(MiqUserRole.allows_any?(@role2.name, :identifiers => ["dashboard_admin", "dashboard_add", "dashboard_view", "policy"])).to eq true
      expect(MiqUserRole.allows_all?(@role2.name, :identifiers => ["dashboard_admin", "dashboard_add", "dashboard_view", "policy"])).to eq true

      expect(MiqUserRole.allows_all?(@role3.name, :identifiers => ["host_show_list", "host_scan", "host_edit"])).to eq true
      expect(MiqUserRole.allows_all?(@role3.name, :identifiers => ["host_view"])).to eq false
      ...
      expect(MiqUserRole.allows_all?(@role3.name, :identifiers => ["vm"])).to eq false
    ...
    it "should return the correct answer calling allows_*_children?" do
      ...
      expect(MiqUserRole.allows_all_children?(@role1.name, :identifier => "dashboard_admin")).to eq true

      expect(MiqUserRole.allows_any_children?(@role2.name, :identifier => "dashboard_admin")).to eq true
      expect(MiqUserRole.allows_all_children?(@role2.name, :identifier => "dashboard_admin")).to eq true

      expect(MiqUserRole.allows_any_children?(@role2.name, :identifier => "everything")).to eq true
      expect(MiqUserRole.allows_all_children?(@role2.name, :identifier => "everything")).to eq true

      expect(MiqUserRole.allows_any_children?(@role1.name, :identifier => "everything")).to eq true
      expect(MiqUserRole.allows_all_children?(@role1.name, :identifier => "everything")).to eq false

      expect(MiqUserRole.allows_any_children?(@role1.name, :identifier => "dashboard")).to eq true
      expect(MiqUserRole.allows_all_children?(@role1.name, :identifier => "dashboard")).to eq false
    end
  end

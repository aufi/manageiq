...
describe Volume do
  context "#volume_group" do
    it "nil when starts with '***physical_'," do
      expect(Volume.new(:volume_group => '***physical_scsi0:0:1').volume_group).to be_false
    end
  ...
  context "#free_space_percent" do
    it "nil when size == nil" do
      expect(Volume.new(:size => nil).free_space_percent).to be_false
    end

    it "nil when size == 0" do
      expect(Volume.new(:size => 0).free_space_percent).to be_false
    end

    it "nil when free_space == nil" do
      expect(Volume.new(:free_space => nil).free_space_percent).to be_false
    end
  ...
  context "#used_space_percent" do
    it "nil when size == nil" do
      expect(Volume.new(:size => nil).used_space_percent).to be_false
    end

    it "nil when size == 0" do
      expect(Volume.new(:size => 0).used_space_percent).to be_false
    end

    it "nil when used_space == nil" do
      expect(Volume.new(:used_space => nil).used_space_percent).to be_false
    end
  ...
  context "#find_disk_by_controller" do
    it "nil when controller NOT like 'scsi0:0:0'" do
      expect(described_class.find_disk_by_controller(double, 'wrong_format')).to be_false
    end

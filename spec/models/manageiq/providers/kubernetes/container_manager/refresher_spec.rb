...
describe ManageIQ::Providers::Kubernetes::ContainerManager::Refresher do
  ...
  def assert_specific_container
    ...
    [
      ...
      expect((date_.kind_of?(ActiveSupport::TimeWithZone) || date_.kind_of?(NilClass))).to be_true
    end
  ...
  def assert_specific_container_group
    ...
    expect(@containergroup.container_replicator).to eq ContainerReplicator.find_by(:name => "monitoring-heapster-controller")
  ...
  def assert_specific_container_node
    ...
    expect(@containernodeconditions.count).to be == 2

...
describe MiqExpression do
  ...
  describe ".atom_error" do
    it "should test atom error" do
      expect(MiqExpression.atom_error("Host-xx", "regular expression matches", '123[)')).not_to be_false

      expect(MiqExpression.atom_error("VmPerformance-cpu_usage_rate_average", "=", '')).not_to be_false
      expect(MiqExpression.atom_error("VmPerformance-cpu_usage_rate_average", "=", '123abc')).not_to be_false
      expect(MiqExpression.atom_error("VmPerformance-cpu_usage_rate_average", "=", '123')).to be_false
      expect(MiqExpression.atom_error("VmPerformance-cpu_usage_rate_average", "=", '123.456')).to be_false
      expect(MiqExpression.atom_error("VmPerformance-cpu_usage_rate_average", "=", '2,123.456')).to be_false

      expect(MiqExpression.atom_error("Vm-cpu_limit", "=", '')).not_to be_false
      expect(MiqExpression.atom_error("Vm-cpu_limit", "=", '123.5')).not_to be_false
      expect(MiqExpression.atom_error("Vm-cpu_limit", "=", '123.5.abc')).not_to be_false
      expect(MiqExpression.atom_error("Vm-cpu_limit", "=", '123')).to be_false
      expect(MiqExpression.atom_error("Vm-cpu_limit", "=", '2,123')).to be_false

      expect(MiqExpression.atom_error("Vm-created_on", "=", Time.now.to_s)).to be_false
      expect(MiqExpression.atom_error("Vm-created_on", "=", "123456")).not_to be_false
    end
  end
  ...
  describe ".quick_search?" do
    it "detects false in hash" do
      expect(MiqExpression.quick_search?(exp)).to be_false
    end

    it "detects in hash" do
      expect(MiqExpression.quick_search?(qs_exp)).to be_true
    end

    it "detects in complex hash" do
      expect(MiqExpression.quick_search?(complex_qs_exp)).to be_true
    end

    it "detects false in miq expression" do
      expect(MiqExpression.quick_search?(MiqExpression.new(exp))).to be_false
    end

    it "detects in miq expression" do
      expect(MiqExpression.quick_search?(MiqExpression.new(qs_exp))).to be_true
    end
  end

  describe "#quick_search?" do
    it "detects false in hash" do
      expect(MiqExpression.new(exp).quick_search?).to be_false
    end

    it "detects in hash" do
      expect(MiqExpression.new(qs_exp).quick_search?).to be_true
    end

    it "detects in complex hash" do
      expect(MiqExpression.new(complex_qs_exp).quick_search?).to be_true
    end
  end

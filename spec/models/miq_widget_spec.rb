...
describe MiqWidget do
  ...
  context "setup" do
    ...
    context "#queue_generate_content_for_users_or_group" do
      ...
      it "admin user" do
        ...
        expect(MiqQueue.exists?(@queue_conditions)).to be_true
      end
      ...
      it "with a task" do
        ...
        expect(MiqQueue.exists?({:method_name => "generate_content"}.merge(@queue_conditions))).to be_true
      end
    end
  ...
  context "# base model" do
    it "should default to enabled" do
      expect(MiqWidget.new.enabled?).to be_true
    end

    it "should not default to read-only" do
      expect(MiqWidget.new.read_only?).to be_false
    end
  end
